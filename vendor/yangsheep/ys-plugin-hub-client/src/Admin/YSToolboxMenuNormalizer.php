...
final class YSToolboxMenuNormalizer {
    ...
    private static function normalize_submenu(): void {
        ...
        // 父選單不存在時，不建立孤兒 submenu 或無父頁的 page hook。
        $parent_exists = false;
        if ( is_array( $menu ) ) {
        ...
        if ( null === $system_entry && class_exists( YSPluginHubClient::class ) ) {
            $before_count = count( $submenu[ self::PARENT_SLUG ] );
            ...
            if ( false !== $hook_suffix && count( $submenu[ self::PARENT_SLUG ] ) > $before_count ) {
                $candidate = array_pop( $submenu[ self::PARENT_SLUG ] );
                if ( is_array( $candidate ) && self::SYSTEM_SLUG === (string) ( $candidate[2] ?? '' ) ) {
